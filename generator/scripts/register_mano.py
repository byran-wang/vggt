...
MANO_DIR_L = "./body_models/MANO_LEFT.pkl"
MANO_DIR_R = "./body_models/MANO_RIGHT.pkl"
...
def fit_frame(seq_name, target_v3d, save_mesh, iteration, is_right,  init_params=None, first_frame=False, use_beta_loss=False):
    target_v3d = torch.FloatTensor(target_v3d.reshape(1, -1, 3)).cuda()

    vis_dir = f"./data/{seq_name}/mesh_fit_vis/"
    ...
    params = optimize_mano_shape(
        target_v3d, mano_layers, optim_specs, iteration, init_params=init_params, use_beta_loss=use_beta_loss
    )
    return params
...
def main():
    ...
    data = np.load(f"./data/{seq_name}/v3d.npy", allow_pickle=True).item()
    ...
    out_p = f"./data/{seq_name}/hold_fit.init.npy"
