...
#
# Round selection (defaults to 01):
#   ROUND=02 bash run_local_preprocess.sh               # data at $HOME/data/rhoi_zed/02
#
...
export ROUND=${ROUND:-01}
export DATASET_DIR=${DATASET_DIR:-$HOME/data/rhoi_zed/$ROUND}
...
# ZED_parse_data() expects svo2 at $DATASET_DIR/<seq>/<seq>.svo2 (nested), but
# the on-disk layout is $DATASET_DIR/<seq>.svo2 (top-level). Hard-link before
# decoding (no extra disk usage; same fs).
# Also point CONDA_DIR at envs/rhoi so {conda_dir}/bin/python3 inside
# ZED_parse_data resolves to the rhoi env (cv2 + pyzed live there).
echo "================================================================"
...
for seq in "${SEQS[@]}"; do
    ...
    src_svo="$DATASET_DIR/$seq.svo2"
    [ -f "$src_svo" ] || { echo "ERROR: $src_svo missing"; exit 1; }
    mkdir -p "$DATASET_DIR/$seq"
    nested_svo="$DATASET_DIR/$seq/$seq.svo2"
    [ -e "$nested_svo" ] || ln "$src_svo" "$nested_svo"
    echo ">>> [P0] $seq: ZED_parse_data"
    CONDA_DIR="$CONDA_BASE/envs/$RHOI_ENV" \
...
echo ""
echo "================================================================"
echo " Local preprocessing DONE. Distribute to local cluster:"
echo "   ROUND=$ROUND bash upload_data_local_cluster.sh"
echo "   ROUND=$ROUND bash run_local_cluster.sh"
echo "================================================================"
