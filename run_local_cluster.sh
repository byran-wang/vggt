...
#
# Usage:
#   bash run_local_cluster.sh                                # WORKERS=l1 l2 l3, SEQS=all
...
LOG_DIR="${LOG_DIR:-$HOME/data/rhoi_zed/logs_$(date +%Y%m%d_%H%M%S)}"
...
if [ "$SKIP_DATA_CHECK" != "1" ]; then
  ...
  for h in "${READY[@]}"; do
    ...
    missing=$(ssh "$h" "for s in $h_seqs; do \
        [ -d ~/data/rhoi_zed/01/\$s/rgb ] || echo \$s; done")
    if [ -n "$missing" ]; then
...
for h in "${READY[@]}"; do
  ...
  echo "  $h: $n seqs = $seqs"
  LOG="$LOG_DIR/${h}.log"
  ( ssh "$h" "nohup bash ~/rhoi/scripts/worker_pipeline.sh $seqs > ~/pipeline_run.log 2>&1 & disown; echo ${h}-pid=\$!" \
      > "$LOG" 2>&1 ) &
done
wait
echo ""
...
echo ""
echo "Tail progress:"
echo "  for h in ${READY[*]}; do echo == \$h ==; ssh \$h 'tail -5 ~/pipeline_run.log'; done"
