...
def _align_frame_with_sam3d(image_info_work, frame_idx, obj_mesh, max_pts=2000, num_iters=100, inlier_thresh=0.3, debug_dir=None):
    ...
    contact_status = _check_contact_rotate_transl_and_reset(rotvec, trans, hand_verts_in_cam, obj_verts, image_info_work, frame_idx, device)
...
def _check_contact_rotate_transl_and_reset(rotvec, trans, hand_verts_in_cam, obj_verts, image_info_work, frame_idx, device,
                             thresh_contact=0.02, thresh_rot=15.0, thresh_trans=0.05):
    """Check initial contact loss and pose distance, reset pose to nearby frame if needed.

    Resets when any of the following hold:
      - contact loss > thresh_contact
      - rotation distance > thresh_rot (degrees) AND translation distance > thresh_trans
    ...
    # Compute rotation and translation distance to nearest registered frame
    ext_curr = image_info_work["extrinsics"][frame_idx]
    R_delta = ext_curr[:3, :3] @ ext_near[:3, :3].T
    rot_dist = np.rad2deg(np.arccos(np.clip((np.trace(R_delta) - 1) / 2, -1.0, 1.0)))
    trans_dist = np.linalg.norm(ext_curr[:3, 3] - ext_near[:3, 3])
    ...
    if hand_verts_in_cam is not None:
        with torch.no_grad():
            ...
            contact_val = contact_loss.item()

    need_reset = False
    reason = []
    if contact_val is not None and contact_val > thresh_contact:
        need_reset = True
        reason.append(f"contact={contact_val:.4f}>{thresh_contact}")
    if rot_dist > thresh_rot and trans_dist > thresh_trans:
        need_reset = True
        reason.append(f"rot={rot_dist:.2f}°>{thresh_rot}° & trans={trans_dist:.4f}>{thresh_trans}")

    contact_str = f"{contact_val:.4f}" if contact_val is not None else "N/A"
    print(f"[align_depth] Frame {frame_idx}: contact={contact_str}, rot={rot_dist:.2f}°, trans={trans_dist:.4f}, nearest={nearest_idx}")
    ...
    if need_reset:
        ...
        print(f"[align_depth] Frame {frame_idx}: reset to nearest registered frame {nearest_idx} ({', '.join(reason)})")
        return "reset"
    else:
        print(f"[align_depth] Frame {frame_idx}: no pose reset needed")
        return "ok"
...
def _check_pose_moved(image_info_work, frame_idx, min_rot=2, min_trans=0.005):
    ...
    print(f"[check_pose_moved] rot={angle:.2f}°, trans={trans:.4f}, nearest={nearest_idx}")
    if angle < min_rot and trans < min_trans:
        print(f"[check_pose_moved] pose barely moved")
        return False
    return True
