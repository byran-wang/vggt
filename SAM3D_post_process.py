...
def load_sam3d_mesh(sam3d_dir: str):
    """Load the mesh generated by ho3d_obj_SAM3D_gen from the SAM3D directory.

    Looks for `scene.glb` (and falls back to `mesh.obj`). The mesh lives in
    SAM3D native object space — the same space `blw2cvc` maps from.

    Returns a trimesh.Trimesh, or None if no mesh file exists.
    """
    candidates = [
        os.path.join(sam3d_dir, "scene.glb"),
        os.path.join(sam3d_dir, "mesh.obj"),
    ]
...
def main(args):
    ...
    mesh_object = load_sam3d_mesh(args.sam3d_dir)
    if mesh_object is None:
        print(f"SAM3D mesh not found in {args.sam3d_dir} (looked for scene.glb, mesh.obj)")
        return
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the output directory of align_SAM3D_pts.py (contains camera.json)",
    )
    parser.add_argument(
        "--sam3d-dir",
        type=str,
        required=True,
        help="Path to the SAM3D directory generated by ho3d_obj_SAM3D_gen (contains scene.glb or mesh.obj)",
    )
    parser.add_argument(
        "--dst-dir",
        type=str,
