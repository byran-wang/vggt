...
#
# Usage:
#   bash upload_data_local_cluster.sh                         # default WORKERS=l1 l2 l3
#   WORKERS="l1 l2 l3" SEQS="001 002 003" bash upload_data_local_cluster.sh
#   SRC_DIR=/custom/path bash upload_data_local_cluster.sh
#
# After success, run:
#   WORKERS="<same list>" SEQS="<same list>" bash run_local_cluster.sh

set -eu

SRC_DIR="${SRC_DIR:-$HOME/data/rhoi_zed/01}"
WORKERS="${WORKERS:-l1 l2 l3}"
SEQS_INPUT="${SEQS:-}"
DST_REMOTE_DIR="${DST_REMOTE_DIR:-data/rhoi_zed/01}"  # relative to remote $HOME
